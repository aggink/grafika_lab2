...
public:
	...
	static void DrawBottom(Point A, Point B, Point C, Point D, Point E, Point F, Point G, vector<Point> pointsBC, vector<Point> pointsCD) {
		glBegin(GL_TRIANGLES);
		DrawTriangles(D, E, F, -1);
		DrawTriangles(F, G, A, -1);
		DrawTriangles(F, A, C, -1);
		...
		size = pointsCD.size();
		for (int i = 0; i < size - 1; i++) {
			DrawTriangles(pointsCD[i], F, pointsCD[i + 1], -1);
		}
		glEnd();
	}
	//закрашиваем вверх
	static void DrawTop(Point A, Point B, Point C, Point D, Point E, Point F, Point G, vector<Point> pointsBC, vector<Point> pointsCD, double z, int alpha) {
		...
		DrawTriangles(D, E, F, z);
		DrawTriangles(F, G, A, z);
		DrawTriangles(F, A, C, z);
		...
		size = pointsCD.size();
		for (int i = 0; i < size - 1; i++) {
			DrawTriangles(pointsCD[i], F, pointsCD[i + 1], z);
		}
	...
	static void DrawABCDEFG(int alpha, bool texChange, GLuint texId[]) {
		...
		Point Center(0, 0, 0);
		double r = 0;
		FindCenterCircle(C, M, D, Center, r);
		vector<Point> pointsCD = Draw::DrawBulge(D, C, z, Center, r);
		glEnd();
